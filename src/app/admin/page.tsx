            ...
            {helpPosts.slice(0, 5).map((p) => (
              <div key={p.id} className="flex items-center justify-between py-2 border-b border-gray-50 last:border-0">
                <div>
                  ...
                  <div className="text-xs text-gray-400">{p.poster} · {p.date}</div>
                </div>
            ...
            {skillPosts.map((p) => {
              ...
              return (
                <div key={p.id} className="flex items-center justify-between py-2 border-b border-gray-50 last:border-0">
                  <div>
                    <div className="text-sm font-medium text-foreground">{p.title}</div>
                    <div className="text-xs text-gray-400">{person?.name} · {p.pricing}</div>
                  </div>
                  <span className="text-xs text-gray-400">🙋 {p.interestedCount}</span>
                </div>
              );
            })}
          </div>
